...
async function seedGradeReleaseConfigs() {
    const configs = [
        {
            gradeId: ID.grades.g1,
            isPublic: false,
            aiGenerationEnabled: false,
            autoRecommendEnabled: false,
            timedExamEnabled: false,
            version: "v0.1.0",
        },
        {
            // MVP: Lớp 2 — fully enabled
    ...
    for (const cfg of configs) {
        await prisma.gradeReleaseConfig.upsert({
            where: { gradeId: cfg.gradeId },
            update: { isPublic: cfg.isPublic, aiGenerationEnabled: cfg.aiGenerationEnabled,
                autoRecommendEnabled: cfg.autoRecommendEnabled, version: cfg.version },
            create: cfg,
        });
    }
    console.log("  ✓ GradeReleaseConfigs (Lớp 2 fully enabled)");
}
// ── 4. Topics (Lớp 2) ──────────────────────────────────────────────────────────
...
async function seedBlueprints() {
    ...
    for (const b of blueprints) {
        ...
        const constraintsJson = constraints ? asJson(constraints) : undefined;
